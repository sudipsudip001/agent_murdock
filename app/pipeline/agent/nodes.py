...
class Nodes:
    ...
    async def agent(self, state: AgentState) -> dict[str, list[AIMessage]]:
        """
        Core reasoning node. LLM looks at the full message history
        and decides: call a tool, or return a final answer.
        """
        logger.info("Agent node entered, message count: %d.", len(state["messages"]))
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    ...
                    response = await self.base_llm.invoke(current_messages)
                else:
